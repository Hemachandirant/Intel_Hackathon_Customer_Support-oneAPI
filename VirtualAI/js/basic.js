...
// Function to fetch response from chat API  
async function fetchChatAPIResponse(userMessage) {  
  ...
  const url = "http://localhost:3000/chat";  // Replace with your chat API URL  
...
function scrollToBottom(elementId) {
  ...
  function speakAndFetchResponse() {
    const userQueryElement = document.getElementById('ChatAIQuery');
    const userQuery = userQueryElement.value;
    console.log("Sending query to ChatAIAI:", userQuery);
  
    // Clear the output box before starting the streaming process
    const ChatAIAIResponseElement = document.getElementById('ChatAIAIResponse');
    ChatAIAIResponseElement.innerText = '';
  
    // Stop any ongoing streaming of processing indicator
    stopProcessingIndicator();
  
    // Start streaming the processing indicator
    processingIndicatorInterval = streamProcessingIndicator('ChatAIAIResponse', '.', 100);
  
    fetchChatAIAIResponse(userQuery)
      .then((response) => {
        ...
        streamResponseText('ChatAIAIResponse', response, 50);
        console.log("Received response from ChatAIAI:", response);
      ...
      .catch((error) => {
        ...
        stopProcessingIndicator();
        console.error('Error fetching ChatAIAI response:', error);
      });
  }
